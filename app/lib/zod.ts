...
export const reservationSchema = object({
  ...
  nik: coerce
    .number({ invalid_type_error: "NIK harus berupa angka" })
    .min(1, "NIK wajib diisi")
    .refine((val) => val.toString().length === 17, "NIK harus terdiri dari 17 digit angka"),
  ...
  destination: string().min(1, "Destination wajib diisi").max(50, "Destination terlalu panjang"),
  date: coerce.date({ invalid_type_error: "Tanggal tidak valid" }).refine((d) => !isNaN(d.getTime()), "Tanggal wajib diisi"),
  estimated_budget: coerce.number({ invalid_type_error: "Estimated Budget harus angka" }).min(0, "Estimated Budget tidak boleh negatif"),
  total_price: coerce.number({ invalid_type_error: "Total Price harus angka" }).min(0, "Total Price tidak boleh negatif"),
